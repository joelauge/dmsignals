...
3. **Devvit app (`dm-radar/`)** — complementary on-Reddit surface, already registered with Reddit (scaffold created via `npm create devvit` on Joel's machine). Daily scheduled scan, Redis dedup, Haiku drafts, digest as Reddit PM, dashboard as a pinnable custom post in a private test subreddit. Requires allowlisting `api.anthropic.com` for HTTP fetch in the Devvit config. Runs as an app account, so it CANNOT read Joel's inbox — reply tracking stays in the Cloudflare worker.

Division of labor: the Worker is the source of truth (email digest, web dashboard, reply tracking, history); the Devvit app is on-Reddit convenience (PM digest, pinned dashboard). Both share identical drafting rules, product context, and the never-auto-post guarantee.
